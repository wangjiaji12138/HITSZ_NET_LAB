...
uint16_t transport_checksum(uint8_t protocol, buf_t *buf, uint8_t *src_ip, uint8_t *dst_ip)
{
    ...
    memcpy(&ip_header, buf->data, sizeof(peso_hdr_t));
    ...
    memcpy(peso_hdr.dst_ip, dst_ip, sizeof(NET_IP_LEN));
    memcpy(peso_hdr.src_ip, src_ip, sizeof(NET_IP_LEN));
    peso_hdr.placeholder = 0;
    peso_hdr.protocol = protocol;
    peso_hdr.total_len16 = swap16(udp_len);
    memcpy(buf->data, &peso_hdr, sizeof(peso_hdr_t));
    ...
    memcpy(buf->data, &ip_header, sizeof(peso_hdr_t));
